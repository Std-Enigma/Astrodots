...
# set shell
if [[ "$(grep "/${USER}:" /etc/passwd | awk -F '/' '{print $NF}')" != "${shellName}" ]]; then
    echo -e "\033[0;32m[SHELL]\033[0m changing shell to ${shellName}..."
    chsh -s "$(which "${shellName}")"
else
    echo -e "\033[0;33m[SKIP]\033[0m ${shellName} is already set as shell..."
fi
